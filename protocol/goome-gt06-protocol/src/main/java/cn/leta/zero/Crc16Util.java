...
public class Crc16Util {
    ...
    /**
     *
     * @param initVal CRC校验时初值
     * @param message 校验内容
     * @return
     */
    public static int doCrc(int initVal, byte [] message) {
        int result  = initVal;
        for (int i = 0; i < message.length; i++) {
            ...
            // 1和2得到的数据再进行异或运算。
            result = (result >> 8) ^ CRC16_TABLE[(result ^ message[i]) & 0xff];
        }
        return  ~result & initVal;
    }

    public static int doCrc(byte [] message) {
        return doCrc(CRC_INIT, message);
    }
}
